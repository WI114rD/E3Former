...
from models.ts2vec.fsnet import TSEncoder, GlobalLocalMultiscaleTSEncoder, MIMO_TSEncoder
...
class net(nn.Module):
    def __init__(self, args, device):
        ...
        encoder = MIMO_TSEncoder(input_dims=args.enc_in + 7, patch_lens=[4, 8, 16, 32], strides=[4, 8, 16, 32],
                                  seq_len=args.seq_len, pred_len=args.pred_len,
                             output_dims=32,  # standard ts2vec backbone value
                             hidden_dims=32, # standard ts2vec backbone value
                             depth=depth) 
        self.encoder = TS2VecEncoderWrapper(encoder, mask='all_true').to(self.device)
        
        self.dim = args.c_out
        
        self.regressor = nn.Linear(32, self.dim).to(self.device)
