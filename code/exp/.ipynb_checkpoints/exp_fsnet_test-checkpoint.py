...
from models.ts2vec.fsnet import TSEncoder, GlobalLocalMultiscaleTSEncoder, MIMO_TSEncoder_MOE
...
class net(nn.Module):
    def __init__(self, args, device):
        ...
        self.regressor = nn.Linear(64, self.dim).to(self.device)
        
    def forward(self, x):
        rep = self.encoder(x)
        print(rep.shape)
        y = self.regressor(rep)
        y_out = rearrange(y, 'b t d -> b (t d)')
        return y_out
    def store_grad(self):
