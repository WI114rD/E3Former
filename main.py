...
def set_seed(seed):
    random.seed(seed)            # Python的内置随机模块
    np.random.seed(seed)         # NumPy的随机模块
    torch.manual_seed(seed)      # 为CPU设置随机种子
    torch.cuda.manual_seed(seed) # 为所有GPU设置随机种子
    # torch.cuda.manual_seed_all(seed) # 为多GPU设置随机种子
    # torch.backends.cudnn.deterministic = True  # 确保CUDA的卷积操作是确定的
    # torch.backends.cudnn.benchmark = False     # 如果网络的输入数据维度或类型上变化不大，置为True能增加运行效率

# set_seed(47) 

torch.cuda.manual_seed(114515)
...
parser.add_argument('--checkpoints', type=str, default='/mnt/bn/jiadong2/checkpoints/', help='location of model checkpoints')
...
parser.add_argument('--patch_lens', type=list, default=[8, 16, 32, 64], help='patch lengths')
parser.add_argument('--stride', type=int, default=8, help='stride')
parser.add_argument('--strides', type=list, default=[8, 16, 32, 64], help='strides')
...
parser.add_argument('--top_k', type=int, default=5, help='top k frequency components for TimesNet')
parser.add_argument('--num_kernels', type=int, default=6, help='for Inception in TimesNet')
parser.add_argument('--gpt_layers', type=int, default=3, help='gpt layers for GPT4TS')
...
print(f"Now the random seed is {args.seed}")
set_seed(args.seed)
...
data_parser = {
    ...
    'FaaS_small': {'data': 'FaaS_small.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7]},
    'FaaS_medium': {'data': 'FaaS_medium.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[21,21,21]},
    'FaaS_large': {'data': 'FaaS_c100.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[100,100,100]},
    'FaaS_c14': {'data': 'FaaS_c14.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[14,14,14]},
    'FaaS_small_transfer': {'data': 'FaaS_small.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7], 'transfer_data': 'FaaS_small_transfer.csv'},
    'FaaS_c14_transfer': {'data': 'FaaS_c14.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[14,14,14], 'transfer_data': 'FaaS_medium_transfer.csv'},
    'ETT_transfer': {'data': 'ETTh1.csv', 'T':'OT', 'S':[1,1,1], 'M':[7, 7, 7], 'transfer_data': 'ETTm1.csv'},
    'ETT_transfer_2': {'data': 'ETTm2.csv', 'T':'OT', 'S':[1,1,1], 'M':[7, 7, 7], 'transfer_data': 'ETTh2.csv'},
    'FaaS_IaaS_transfer': {'data': 'FaaS_small.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7], 'transfer_data': 'IaaS_c7.csv'},
    'IaaS_FaaS_transfer': {'data': 'IaaS_c7.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7], 'transfer_data': 'FaaS_small.csv'},
    'IaaS_c7': {'data': 'IaaS_c7.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7]},
    'IaaS_c7_transfer': {'data': 'IaaS_c7.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7], 'transfer_data': 'IaaS_c7_transfer.csv'},
    'IaaS_c14': {'data': 'IaaS_c14.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[14,14,14]},
    'IaaS_c20': {'data': 'IaaS_c20.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[20,20,20]},
    'IaaS_cpu_user_c7': {'data': 'IaaS_cpu_user_c7.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7]},
    'IaaS_cpu_user_c7_transfer': {'data': 'IaaS_cpu_user_c7.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7], 'transfer_data': 'IaaS_cpu_user_c7_transfer.csv'},
    'IaaS_cpu_user_c14': {'data': 'IaaS_cpu_user_c14.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[14,14,14]},
    'IaaS_cpu_user_c20': {'data': 'IaaS_cpu_user_c20.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[20,20,20]},
    'ETTh1_test': {'data': 'ETTh1.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7]},
    'ETTh2_test': {'data': 'ETTh2.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7]},
    'ETTm1_test': {'data': 'ETTm1.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7]},
    'ETTm2_test': {'data': 'ETTm2.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7]},
    'IaaS_c7_new': {'data': 'IaaS_cpu_user_c7_period.csv', 'T':'cluster_0', 'S':[1,1,1], 'M':[7,7,7]},
}
if args.data in data_parser.keys():
    data_info = data_parser[args.data]
    args.data_path = data_info['data']
    if 'transfer_data' in data_info.keys():
        args.transfer_data_path = data_info['transfer_data']
    else:
        args.transfer_data_path = data_info['data']
...
for ii in range(args.itr):
    ...
    setting = '{}_{}_{}_pl{}_ol{}_opt{}_tb{}_{}'.format(method_name, args.data, args.seq_len, args.pred_len,args.online_learning, args.opt, args.test_bsz, suffix)
    ...
    print(f"Online inference costs: {m[-1]}s")
    metrics.append(m)
...
folder_path = '/mnt/bn/jiadong2/Online_results/' + setting + '/'
