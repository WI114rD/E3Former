...
from models.ts2vec.fsnet import TSEncoder, GlobalLocalMultiscaleTSEncoder, MIMO_TSEncoder, MIMO_TSEncoder_MOE
...
class net(nn.Module):
    def __init__(self, args, device):
        ...
        # print(args.enc_in)
        encoder = MIMO_TSEncoder_MOE(input_dims=args.enc_in + 7, patch_lens=[4, 8, 16, 32], strides=[4, 8, 16, 32],
    ...
    def forward_weight(self, x, x_mark, g1, g2):
        ...
        x = torch.cat([x, x_mark], dim=-1)
        rep2 = self.encoder(x)
        z = self.regressor(rep2)
        y2 = rearrange(z, 'b t d -> b (t d)')
